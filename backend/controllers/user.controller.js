...
export const registerUser = async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO usuario (nombre, apellido, correo, password, id_rol)
      VALUES (?, ?, ?, ?, ?)
    `;

    await db.query(query, [nombre, apellido, correo, hashedPassword, id_rol]);
...
export const updateUsuario = async (req, res) => {
  try {
    ...
    const query = `
      UPDATE usuario
      SET nombre = ?, apellido = ?, correo = ?, ${password ? "password = ?," : ""} id_rol = ?
      WHERE id_usuario = ?
    `;

    const params = password
      ? [nombre, apellido, correo, hashedPassword, id_rol, id]
